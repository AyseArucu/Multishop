...
import { FiHeart, FiArrowRight, FiEye, FiStar, FiShoppingCart } from 'react-icons/fi';
...
export default function CategoryProductsSection({ pageSettings }: { pageSettings?: any }) {
    ...
    <div className="bg-[#fafafa]">
      {categories.map((category: any, index: number) => (
        <motion.section 
          ...
          className={`py-10 md:py-16 ${index > 0 ? 'border-t border-gray-200/50' : ''}`}
        >
          <div className="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between mb-12">
              ...
              <Link href={category.id === 'mixed-category' ? '/products' : `/products?category=${category.id}`} className="text-pink-500 font-bold hover:text-pink-600 flex items-center gap-2 group transition-colors">
                Tümünü Gör 
              ...
              {category.products.map((product: any, prodIndex: number) => (
                <SwiperSlide key={product.id} className="pb-4">
                  <motion.div
                    ...
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ delay: prodIndex * 0.05, duration: 0.5 }}
                    className="h-full"
                  >
