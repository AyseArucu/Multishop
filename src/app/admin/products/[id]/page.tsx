...
export default function EditProductPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setCategories(catData || []);
      const product = prodData.products?.find((p: any) => p.id === params.id);
      ...
      if (product) {
        setFormData({
          ...
          showVideoInSlider: product.showVideoInSlider !== false,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!formData.categoryId) {
       alert("Lütfen önce bir kategori seçin veya oluşturun!");
       return;
    }
  ...
  if (loading) return <div className="p-12 text-center text-gray-500 font-medium">Ürün bilgileri yükleniyor...</div>;
    ...
    <div className="max-w-5xl mx-auto p-4 sm:p-8 min-h-screen pb-24">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 font-medium">Mevcut ürünün detaylarını güncelliyorsunuz.</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-8">
           ...
           >
             <FiSave size={24} /> {saving ? 'Kaydediliyor...' : 'Değişiklikleri Kaydet'}
           </button>
        </div>
      </form>
    </div>
